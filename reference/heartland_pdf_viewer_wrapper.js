...
#divider {
  margin: 0 var(--page-selector-spacing);
    ...
    </div><!--_html_template_end_-->`}static get properties(){return{docLength:{type:Number,value:1,observer:"docLengthChanged_"},pageNo:{type:Number,value:1}}}get pageSelector(){return this.$.pageselector}pageNoCommitted(){const page=parseInt(this.pageSelector.value,10);if(!isNaN(page)&&page<=this.docLength&&page>0){this.dispatchEvent(new CustomEvent("change-page",{detail:{page:page-1,origin:"pageselector"},composed:true}))}else{this.pageSelector.value=this.pageNo.toString()}this.pageSelector.blur()}docLengthChanged_(){const numDigits=this.docLength.toString().length;this.style.setProperty("--page-length-digits",`${numDigits}`)}select(){this.pageSelector.select()}isActive(){return this.shadowRoot.activeElement===this.pageSelector}onInput_(){this.pageSelector.value=this.pageSelector.value.replace(/[^\d]/,"")}}customElements.define(ViewerPageSelectorElement.is,ViewerPageSelectorElement);var MAX_RADIUS_PX=300;var MIN_DURATION_MS=800;var distance$1=function(x1,y1,x2,y2){var xDelta=x1-x2;var yDelta=y1-y2;return Math.sqrt(xDelta*xDelta+yDelta*yDelta)};Polymer({_template:html`<!--css-build:shadow--><style scope="paper-ripple">:host {
